...
-spec fetch_all_(Table :: p2phun_types:table()) -> [#peer{}].
fetch_all_(Table) -> [Peer || [Peer] <- ets:match(Table, '$1')].

-spec fetch_peer_(PeerId :: p2phun_types:id(), Table :: p2phun_types:table()) -> [#peer{}].
fetch_peer_(PeerId, Table) -> ets:lookup(Table, PeerId).

-spec sudo_add_peers_(Peers :: [#peer{}], Table :: p2phun_types:table()) -> true.
sudo_add_peers_(Peers, Table) -> ets:insert(Table, Peers).

-spec delete_peers_(Peers :: [p2phun_types:id()], Table :: p2phun_types:table()) -> ok.
delete_peers_(Peers, Table) ->
    lists:foreach(fun(Peer) -> ets:delete(Table, Peer) end, Peers).

-spec fetch_all_peers_to_ask_for_peers_(Table :: p2phun_types:table(), MaxTimeSinceLastSpoke :: integer()) -> [#peer{}].
...
-spec fetch_all_peers_to_ping_(Table :: p2phun_types:table(), MaxTimeSinceLastSpoke :: integer()) -> [#peer{}].
...
-spec fetch_last_fetched_peer_(Table :: p2phun_types:table(), PeerId :: p2phun_types:id()) -> integer().
fetch_last_fetched_peer_(Table, PeerId) ->
    [Peer] = ets:lookup(Table, PeerId),
    Peer#peer.last_fetched_peer.

-spec fetch_peers_closest_to_id_and_not_processed(Table :: p2phun_types:table(), Id :: p2phun_types:id(), MaxDist :: integer(), MaxVals :: non_neg_integer()) -> [#peer{}].
...
-spec fetch_peers_closest_to_id_(Table :: p2phun_types:table(), Id :: p2phun_types:id(), MaxDist :: integer(), MaxVals :: non_neg_integer()) -> [#peer{}].
...
-spec fetch_peers_closest_to_id_1(Table :: p2phun_types:table(), Id :: p2phun_types:id(), MaxVals :: non_neg_integer(), MatchSpec :: ets:match_spec()) -> [#peer{}].
fetch_peers_closest_to_id_1(Table, Id, MaxVals, MatchSpec) ->
